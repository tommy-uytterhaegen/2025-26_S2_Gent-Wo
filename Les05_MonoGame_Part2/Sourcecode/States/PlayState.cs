...
namespace MonoGame_Pikachu.States
{
    public class PlayState : AbstractState
    {
        ...
        public override void Draw(GameTime gameTime, SpriteBatch spriteBatch)
        {
            ...
            foreach (var enemyPosition in Context.EnemyPositions)
                spriteBatch.Draw(Context.SharkTexture, enemyPosition, Game1.SHARK_SCALE);

            spriteBatch.Draw(Context.PlayerTexture, Context.PlayerPosition, Game1.PLAYER_SCALE);
        }
        ...
        private void UpdatePlayerPosition()
        {
            ...
            if (ShouldGoRight())
                Context.PlayerPosition = Context.PlayerPosition with { X = Context.PlayerPosition.X + Game1.PLAYER_SPEED };

            if (IsKeyDown(Keys.Left, Keys.Q))
                Context.PlayerPosition = Context.PlayerPosition with { X = Context.PlayerPosition.X - Game1.PLAYER_SPEED };

            if (IsKeyDown(Keys.Up, Keys.Z))
                Context.PlayerPosition = Context.PlayerPosition with { Y = Context.PlayerPosition.Y - Game1.PLAYER_SPEED };
        ...
        {
            for (var i = 0; i < Context.EnemyPositions.Count; i++)
            {
                Context.EnemyPositions[i] = Context.EnemyPositions[i] with { X = Context.EnemyPositions[i].X - Game1.SHARK_SPEED };
            }
        }
        ...
        private void SpawnEnemyIfNeeded(GameTime gameTime)
        {
            ...
            if (Context.ElapsedTimeForSpawnIsMs >= Game1.SHARK_SPAWN_TIME_IN_MS)
            {
                ...
                Context.EnemyPositions.Add(new Vector2(Context.Graphics.PreferredBackBufferWidth, Context.Graphics.PreferredBackBufferHeight / 2));
